...
# ── 5. Per-device DNS instructions ─────────────────────────────────────
PRIMARY_IP=$(echo $LAN_IPS | awk '{print $1}')
bold "5. Point your devices at this Mac as DNS server"
echo
echo "   This Mac's IP: $PRIMARY_IP"
echo
echo "   Router-wide (best): set primary DNS to $PRIMARY_IP in your"
echo "     router's DHCP settings. Every device picks it up next lease."
echo
echo "   Per-device fallback:"
echo "     macOS:   System Settings → Network → Details → DNS → +$PRIMARY_IP"
echo "     iPhone:  Settings → Wi-Fi → tap (i) → Configure DNS → Manual"
echo "              → Add Server → $PRIMARY_IP"
echo "     Windows: Settings → Network → Wi-Fi → Hardware properties →"
echo "              DNS server → Edit → $PRIMARY_IP"
echo

# ── 6. iPhone CA install instructions ──────────────────────────────────
bold "6. iPhone setup (install root CA so Safari trusts the cockpit)"
echo
